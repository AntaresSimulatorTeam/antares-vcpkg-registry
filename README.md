...
#### Normally: cutting a release seeds the mirror

`seed.yml` is a **reusable** workflow, and the intended trigger is a release.
`Antares_Simulator`'s `new_release.yml` calls it once per OS from a `seed_asset_mirror`
job, gated on a dispatch input, in the same shape as the existing `generate_results` job:
...
something that has to stay buildable for years. Patch releases on a maintenance branch
re-run it, so a branch that is still being maintained keeps getting re-checked for free,
and a newly cut branch is covered by its first release rather than by someone remembering
to add it somewhere.
...
The run summary reports, per call, the baseline it resolved, how many assets were new, and
— the part worth reading — any archive whose origin already returns 404. Those are the ones
only the manual recovery procedure below can bring back.

#### One-off: dispatching it here

[`Seed asset mirror`](../../actions/workflows/seed.yml) can also be dispatched directly on
this repository with the consumer repo, the ref, the runner OS and the triplet. No token
setup is involved on this path — the job's own `GITHUB_TOKEN` has the access. Use it to
seed a ref that is not going through a release: after a baseline bump on `develop`, or to
backfill a branch released before this existed.
